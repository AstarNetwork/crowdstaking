...
import { Crowdstake } from "./typechain/Crowdstake";
import { Crowdstake__factory } from "./typechain/factories/Crowdstake__factory";
import { CrowdstakeFactory } from "./typechain/CrowdstakeFactory";
import { CrowdstakeFactory__factory } from "./typechain/factories/CrowdstakeFactory__factory";
...
export const CrowdstakeContext = React.createContext<SymfoniCrowdstake>(emptyContract);
export const CrowdstakeFactoryContext = React.createContext<SymfoniCrowdstakeFactory>(emptyContract);
...
export interface SymfoniCrowdstake {
    instance?: Crowdstake;
    factory?: Crowdstake__factory;
}

export interface SymfoniCrowdstakeFactory {
    instance?: CrowdstakeFactory;
    factory?: CrowdstakeFactory__factory;
}
...
export const Symfoni: React.FC<SymfoniProps> = ({
    ...
    const [Crowdstake, setCrowdstake] = useState<SymfoniCrowdstake>(emptyContract);
    const [CrowdstakeFactory, setCrowdstakeFactory] = useState<SymfoniCrowdstakeFactory>(emptyContract);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const doAsync = async () => {
            ...
            const finishWithContracts = (text: string) => {
                setCrowdstake(getCrowdstake(_provider, _signer))
                setCrowdstakeFactory(getCrowdstakeFactory(_provider, _signer))
                finish(text)
            }
    ...
    const getCrowdstake = (_provider: providers.Provider, _signer?: Signer) => {
        let instance = _signer ? Crowdstake__factory.connect(ethers.constants.AddressZero, _signer) : Crowdstake__factory.connect(ethers.constants.AddressZero, _provider)
        const contract: SymfoniCrowdstake = {
            instance: instance,
            factory: _signer ? new Crowdstake__factory(_signer) : undefined,
        }
        return contract
    }
        ;
    const getCrowdstakeFactory = (_provider: providers.Provider, _signer?: Signer) => {
        let instance = _signer ? CrowdstakeFactory__factory.connect(ethers.constants.AddressZero, _signer) : CrowdstakeFactory__factory.connect(ethers.constants.AddressZero, _provider)
        const contract: SymfoniCrowdstakeFactory = {
            instance: instance,
            factory: _signer ? new CrowdstakeFactory__factory(_signer) : undefined,
        }
        return contract
    }
        ;
            ...
            <ProviderContext.Provider value={[provider, setProvider]}>
                <SignerContext.Provider value={[signer, setSigner]}>
                    <CurrentAddressContext.Provider value={[currentAddress, setCurrentAddress]}>
                        <CrowdstakeContext.Provider value={Crowdstake}>
                            <CrowdstakeFactoryContext.Provider value={CrowdstakeFactory}>
                                {showLoading && loading ?
                            ...
                            </CrowdstakeFactoryContext.Provider >
                        </CrowdstakeContext.Provider >
